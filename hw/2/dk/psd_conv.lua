...
cmd:text()
cmd:text('Options')
cmd:option('-dir','outputs.psd', 'subdirectory to save experiments in')
cmd:option('-data','5','Data set file')
cmd:option('-seed', 123211, 'initial random seed')
cmd:option('-inputsize',96, 'size of input patches')
...
cmd:option('-etadecayinterval',4000,'learning rate decay interval')
cmd:option('-maxiter',4000,'max number of updates')
cmd:option('-statinterval',4000,'interval for saving stats and models')
...
if not paths.filep('/home/ubuntu/2/clumps/' .. params.data .. '.t7b') then
...
--imutils.mapdata(params.data)
--data:conv()

-- data = torch.load('../clumps/1.t7b')
...
mlp = torch.load('out4.psd/psd,encoderType=tanh,kernelsize=3,lambda=1/model.bin')
