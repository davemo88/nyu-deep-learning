...
\subsubsection{t-SNE}
We ran t-SNE on the output of each of the stacked encoders. As one can see from the following three images, visually defined clusters begin to emerge after the third encoder. Recall that our final model did not use any unsupervised pre-training and so these clusters come purely supervised training.
\begin{figure}[H]
	\centering
