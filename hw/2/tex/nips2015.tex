...
\title{Assignment 2 Deep Learning Spring 2016}
...
Our final model used only supervised training as our attempts to leverage the unlabeled data were not entirely successful. However working with the unlabeled data did inspire the architecture of our final model which achieved a modest $\sim70\%$ accuracy (see \autoref{app:A}).


We tried to use the unlabeled data in two ways. First, we tried to build our own stacked what-where convolutional autoencoders (CAE) from scratch, starting with \autocite{zhao2015stacked}.  Second, we used the \texttt{unsup} package to train a PSD Convolutional Autoencoder \cite{psdconv}. We spent the vast majority of our time working on these and describe them later in more detail.
...
We included filters from each of the three coders (see \autoref{app:F}). One can see that the filters have some structure, e.g. the first encoder's filters look like the desired edges and color blobs.

\subsubsection{t-SNE}
We ran t-SNE on the output of each of the stacked encoders.  As one can see from the figures in \autoref{app:D}, visually identifiable clusters emerge especially from the second and third encoder outputs. Recall that our final model did not use any unsupervised pre-training and so these clusters come from purely supervised training.
...
Following loosely the example of several papers \autocite{zhao2015stacked,masci2011stacked}, we built a stacked CAE that was trained to minimize reconstruction error. In \autoref{app:B}, the full model architecture is given verbatim. Our architecture was based on two building blocks:
\begin{itemize}
...
\appendix
\section{Kaggle Performance}\label{app:A}
\begin{figure}[H]
		\centering
...
\section{Stacked CAE Architecture}\label{app:B}
\begin{verbatim}
nn.Sequential {
  [input -> (1) -> (2) -> (3) -> output]
  (1): nn.Sequential {
...
\end{verbatim}
\section{Filters}\label{app:F}

\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth, keepaspectratio=true]{filters-1}
	\caption{filters from convolutional layer of first encoder}
	\label{fig:filters-1}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth, keepaspectratio=true]{filters-2}
	\caption{filters from convolutional layer of second encoder}
	\label{fig:filters-2}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth, keepaspectratio=true]{filters-3}
	\caption{filters from convolutional layer of third encoder}
	\label{fig:filters-3}
\end{figure}
...
\section{Augmentations}\label{app:E}
\begin{figure}[H]
  \centering
